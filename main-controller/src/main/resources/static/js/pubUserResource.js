...
const show_res_table_id_vue = new Vue({
  ...
  methods: {
    indicatorCurPAge: function () {
      ...
      queryPubResource(queryUrl,
          {offset: offset, numberOfPage: numberOfPage, id: id});
    },
    ...
    download: function (index) {
      ...
      $.post(insertDownloadResUrl,
          {rn: resourcesUrl[index], id: id}, function () {
  ...
  $.post(url, data, function (data) {
    ...
    if (data && data.ids.length > 0) {
      ...
      for (let resourcesUrlElement of resourcesUrl) {
        ...
        dateTime.push(date.getFullYear() + '.' + (date.getMonth() + 1) + '.'
